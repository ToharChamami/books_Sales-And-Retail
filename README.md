...
This system was d
esigned to manage the sales and retail department of a national bookstore chain.
...
# דוח שלב ה' - ממשק משתמש (GUI)
**מגישות:** [שם משפחה ושם פרטי 1 - ת.ז] , [שם משפחה ושם פרטי 2 - ת.ז]

### 1. מבוא
בשלב זה פיתחנו ממשק משתמש גרפי מלא לאפליקציית ניהול רשת הספרים. 
השתמשנו ב-Python יחד עם ספריית `customtkinter` לעיצוב הממשק, `psycopg2` להתחברות מול בסיס הנתונים PostgreSQL, ו-`python-bidi` לסידור העברית.
המערכת פוצלה ל-5 קבצי קוד כדי לשמור על סדר, כאשר קובץ `main_app.py` הוא מסך הניווט הראשי.

### 2. הוראות הפעלה
1. התקנת ספריות נדרשות: `pip install psycopg2-binary customtkinter python-bidi`
2. במידת הצורך, עדכון פרטי התחברות לשרת בקובץ `db_connection.py`.
3. הרצת המערכת מהטרמינל: `python main_app.py`.

### 3. צילומי מסך והוכחות ביצוע

**מסך הכניסה הראשי:**
![מסך ראשי](DBProject%20215108549-210042453/שלב%20ה/מסך%20ראשי.png)

**מסך ניהול ספרים (CRUD):**
* שליפת הנתונים (READ):
![בדיקת קריאה](DBProject%20215108549-210042453/שלב%20ה/בדיקת%20קריאה.png)
* יבוא נתונים לעדכון לפי מפתח:
![בדיקת יבוא נתונים](DBProject%20215108549-210042453/שלב%20ה/בדיקת%20יבוא%20נתונים.png)
* יצירת ספר חדש:
![בדיקת יצירה](DBProject%20215108549-210042453/שלב%20ה/בדיקת%20יצירה.png)
* מחיקת ספר:
![בדיקת מחיקה](DBProject%20215108549-210042453/שלב%20ה/בדיקת%20מחיקה.png)

**מסך דוחות (שלב ב):**
שאילתות הכוללות JOIN להצגת שמות במקום מזהים:
![דוח מלאי מרוכז](DBProject%20215108549-210042453/שלב%20ה/דוח%20מלאי%20מרוכז%20לפי%20סניפים.png)

**מסך פעולות אוטומטיות (שלב ד):**
* ביצוע מכירה תקינה:
![בדיקת מכירה](DBProject%20215108549-210042453/שלב%20ה/בדיקת%20מכירה.png)
* טיפול בחריגה מהשרת (ניסיון למכור ללא מלאי):
![שגיאת מלאי](DBProject%20215108549-210042453/שלב%20ה/בדיקת%20מכירה-%20ללא%20מלאי%20מספיק.png)